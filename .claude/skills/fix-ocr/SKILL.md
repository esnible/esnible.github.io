...
5. **Apply edits with the `Edit` tool**, one logical fix at a time. For each edit:
   ...
   - Never change quoted historical spellings or transliterations you cannot verify — when unsure, leave it. **This applies with extra force to personal names in archival material** (correspondence, official records, mint-official rosters): real historical documents genuinely used inconsistent spelling for the same person (clerks, transliteration drift), so two different spellings of a name are not on their own evidence of OCR error. Occurrence count and "which spelling sounds more like a real name" are not enough — only merge two spellings of a personal name when there's an actual visual-confusion mechanism (see "Common OCR error patterns") tying the specific letters together, and default to leaving both as they are OCR'd when that's absent.
